...
import { Clapperboard, HardDriveDownload, LoaderCircle } from "lucide-react";
        ...
        <LaserCardFrame tier={null} shape="landscape">
          ...
          >
            {isRunning ? (
              <span
                aria-hidden
                className="pointer-events-none absolute inset-x-0 top-0 z-20 h-px bg-gradient-to-r from-transparent via-accent/50 to-transparent"
              />
            ) : null}
                ...
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  >
                    {isRunning ? (
                      ...
                      <HardDriveDownload className="h-3 w-3" strokeWidth={1.75} aria-hidden />
                    )}
                    {meta.label}
                  </span>
                </div>
                ...
                {isRunning && progress != null ? (
                  <div className={`space-y-1.5 ${compact ? "mt-2" : "mt-3"}`}>
                    ...
                    <div className="h-1 overflow-hidden rounded-full bg-bg-deep/80 ring-1 ring-inset ring-border/60">
                      <div
                        className="h-full rounded-full bg-gradient-to-r from-accent/80 to-accent-bright transition-[width] duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]"

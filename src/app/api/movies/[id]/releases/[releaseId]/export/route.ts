...
export async function POST(request: NextRequest, context: ReleaseRouteContext) {
  ...
  try {
    if (body.dryRun) {
      const result = await exportReleaseDryRun(release, movie, body.filename);
    ...
    const result = await exportReleaseToMediaDir(
      release,
      movie,
      body.filename,
    );
    ...
    return mapDomainError(err, "Не удалось экспортировать релиз");
  }
}

...
CREATE TABLE "new_ReleaseBuild" (
    ...
    CONSTRAINT "ReleaseBuild_moviePartId_fkey" FOREIGN KEY ("moviePartId") REFERENCES "MoviePart" ("id") ON DELETE SET NULL ON UPDATE CASCADE,
...
INSERT INTO "new_ReleaseBuild" ("acknowledgedWarnings", "cancelRequested", "createdAt", "errorMessage", "externalStorageId", "finishedAt", "heartbeatAt", "id", "movieId", "outputPath", "outputReleaseId", "outputReleaseType", "outputVersion", "phase", "progressDurationMs", "progressMessage", "progressOutTimeMs", "progressPercent", "progressSpeed", "progressStepIndex", "progressStepTotal", "queueOrder", "requiresTranscode", "startedAt", "status", "updatedAt") SELECT "acknowledgedWarnings", "cancelRequested", "createdAt", "errorMessage", "externalStorageId", "finishedAt", "heartbeatAt", "id", "movieId", "outputPath", "outputReleaseId", "outputReleaseType", "outputVersion", "phase", "progressDurationMs", "progressMessage", "progressOutTimeMs", "progressPercent", "progressSpeed", "progressStepIndex", "progressStepTotal", "queueOrder", "requiresTranscode", "startedAt", "status", "updatedAt" FROM "ReleaseBuild";
...
CREATE INDEX "ReleaseBuild_kind_status_idx" ON "ReleaseBuild"("kind", "status");

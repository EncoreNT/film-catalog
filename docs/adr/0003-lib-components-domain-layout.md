...
Одноразовые codemod-скрипты: `scripts/move-lib.mjs`, `scripts/move-components.mjs` (удалены после миграции). Shims на старых путях **не** оставлялись.
...
**Follow-ups (second pass, 2026 Q3):**

- Удалены одноразовые codemod-скрипты `scripts/move-lib.mjs`, `scripts/move-components.mjs`.
